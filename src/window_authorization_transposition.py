...
class Window_authorization_transposition(QWidget):
    def __init__(self):
        super().__init__()
    ...
    def start_test(self):
        from window_transposition_test import Window_transposition_test
        name = self.input.text().strip()
        if not name:
            QMessageBox.warning(self, 'Ошибка', 'Пожалуйста, введите имя')
            return
        self.close()
        self.testWindow = Window_transposition_test(name)
